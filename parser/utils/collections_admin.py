...
import json
import config
from utils.main import get_approved_account_ids
...
async def add_collection_to_approved(contract_address: str):
    data = {
        "ids": [contract_address]
    }
    approved_account_ids = await get_approved_account_ids()
    if contract_address not in approved_account_ids:
        async with aiohttp.ClientSession() as client:
            await client.post(
                f'http://{config.main_server_host}:{config.api_tx_sing_port}/add_approved_nft_contract_ids',
                json=data)
