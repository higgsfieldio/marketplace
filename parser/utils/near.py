...
async def insert_new_token_or_update_existed(token: dict, contract: str, transaction_hash: Optional[str] = None,
                                             royalty: Optional[dict] = None, sem_send_to_main_serv: Optional = None):
    ...
    contract_address = await main_additional_funcs.get_contract_id(obj, contract)
    if contract_address is not None:
        new_token_dict['$set']['collection_id'] = contract_address
    ...
    if obj is not None:
        ...
        if contract_address is not None:
            update_queries['tokens'] = [dict(
                type='update_one',
                find_query={"_id": obj['_id'], 'collection_id': {"$ne": contract_address}},
            ...
            update_queries['tokens'] = [dict(
                ...
                update_query=new_token_dict)]
        if is_staked_info is not None:
            print(f'is_staked_info: {update_queries}')
    ...
    for i in range(5):
        try:
            async with sem_send_to_main_serv:
                async with aiohttp.ClientSession() as client:
...
async def get_nft_supply_for_owner(item: str, account_id: str):
    params = {
        "jsonrpc": "2.0",
        "id": "dontcare",
        "method": "query",
        "params": {
            ...
            "args_base64": base64.b64encode(
                f'{{"account_id":"{account_id}"}}'.encode("ascii")
            ).decode('ascii'),
            "finality": "final"}
    }
    async with aiohttp.ClientSession() as client_1:
...
async def get_nft_from_contract_stepped(contract: str, account_id: str, from_index: int, limit: int, base_uri: str,
                                        sem_send_to_main_serv):
    update_queries = dict()
    params = {
        "jsonrpc": "2.0",
        "id": "dontcare",
        ...
        "params": {
            ...
            "args_base64": base64.b64encode(
                f'{{"account_id":"{account_id}","from_index":"{from_index}","limit":'
                f'{limit}}}'.encode("ascii")
            ).decode('ascii'),
            "finality": "final"}
    }
    async with aiohttp.ClientSession() as client_1:
        async with client_1.post(config.main_near_link,
            ...
            if resp_1.status == 200:
                if 'error' not in response and 'error' not in response['result']:
                    tokens = json.loads(bytearray(response['result']['result']).decode('utf-8'))
                    for token in tokens:
                        try:
                            token = tokens_models.Token(**token).dict()
                            royalty = await get_royalty(contract, token['token_id'], token['owner_id'])
                                ...
                                if 'reference' in token['metadata'] and token['metadata']['reference'] is not None:
                                    if not token['metadata']['reference'].lstrip('/')[:7] in ['https:/', 'http://']:
                                        token['metadata']['reference'] = base_uri.rstrip('/') + \
                                                                         '/' + token['metadata']['reference'].lstrip(
                                            '/')
                            update_query = await insert_new_token_or_update_existed(token, contract, None, royalty,
                                                                                    sem_send_to_main_serv)
                            if update_query is not None:
                                for key in update_query.keys():
                                    if key not in update_queries:
                                        update_queries[key] = list()
                                    [update_queries[key].append(elem) for elem in update_query[key]]
                        except Exception as e:
                            print(e, token)
                elif 'GasLimitExceeded' in response['result']['error'] and limit != 5:
                    task_list = list()
                    step = 5
                    for i in range(from_index, from_index + limit, step):
                        task_list.append(asyncio.create_task(get_nft_from_contract_stepped(contract, account_id,
                                                                                           i, step, base_uri,
                                                                                           sem_send_to_main_serv)))
...
async def get_nft_from_contract(item: str, account_id: str, init_limit: int, base_uri: str,
                                sem_send_to_main_serv):
    update_queries = dict()
    task_list = list()
    step = 15
    for i in range(0, init_limit, step):
        task_list.append(asyncio.create_task(get_nft_from_contract_stepped(item, account_id, i, step, base_uri,
                                                                           sem_send_to_main_serv)))
...
async def get_items(account_id: str, sem_send_to_main_serv):
    update_queries = dict()
    async with aiohttp.ClientSession() as client:
        async with client.get(f'{config.main_near_helper_link}/account/{account_id}/likelyNFTs') as resp:
            if resp.status == 200:
                response = json.loads(await resp.text())
                task_list = list()
                for item in response:
                    init_limit = await get_nft_supply_for_owner(item, account_id) + 1
                    params = {
                        "jsonrpc": "2.0",
                        "id": "dontcare",
                        "method": "query",
                    ...
                    async with aiohttp.ClientSession() as client_1:
                        async with client_1.post(config.main_near_link,
                            ...
                            if resp_1.status == 200 and 'error' not in response and 'error' not in response['result']:
                                ...
                                base_uri = ''
                                if 'base_uri' in loaded_js:
                                    base_uri = loaded_js['base_uri']

                                task_list.append(asyncio.create_task(get_nft_from_contract(item, account_id,
                                                                                           init_limit, base_uri,
                                                                                           sem_send_to_main_serv)))
                items_gathered = await asyncio.gather(*task_list)
                for item_gathered in items_gathered:
