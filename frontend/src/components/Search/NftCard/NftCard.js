...
function NftCard({ item, user, toggleLike, loggedIn, currentUser, isCreatingPage, creatingPrice, setLoginPopupOpened, isExplictAccept, setExplictAccept , usdExchangeRate}) {
    ...
    useEffect(() => {
        ...
        if (currentUser) {
            setLiked(currentUser ? currentUser.liked && item && currentUser.liked.filter((itm) => {
                if (itm.token_id === item._id) return true
                else return false
            }).length > 0 : false)
        }
    ...
    return (
        <div className='nft-card'>
            <div className='nft-card__owner'>
                ...
                <div className={`nft-card__item-likes `} onClick={handleToggleLike}>
            ...
            {/* <Link className='nft-card__item-preview' to={`/item/${item && item._id}/sale-history`}>
                ...
                <ImageOnLoad className='nft-card__item-preview-img' src={`${API_LINK}/tokens/get_file/${item.preview_url.size0}`} keyValue={`${API_LINK}/tokens/get_file/${item.preview_url.size0}`} alt='item preview'></ImageOnLoad>
            </Link> */}
            ...
            <Link  className='nft-card__link' to={`/item/${item && item._id}/sale-history`}>
