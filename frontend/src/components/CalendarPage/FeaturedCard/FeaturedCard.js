...
function FeaturedCard({ timerValue, card, usdExchangeRate }) {


    const [timeLeftValue, setTimeLeftValue] = useState(card.timestamp ? {
        days: Math.floor((card.timestamp - moment().format('X')) / (3600 * 24)),
        hours: Math.floor((card.timestamp - moment().format('X')) % (3600 * 24) / 3600),
        minutes: Math.floor((card.timestamp - moment().format('X')) % 3600 / 60),
        seconds: Math.floor((card.timestamp - moment().format('X')) % 60),
    } : {});


    useEffect(() => {
        let seconds = card.timestamp - moment().format('X')
        setTimeLeftValue({
