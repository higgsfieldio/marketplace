...
function EditCollection({ currentUser, loggedIn, setCurrentUser }) {
    ...
    const [coverFileUrlValue, setCoverFileUrlValue] = useState(collection ? `${API_LINK}/collections/get_file/${collection.cover_url.size0}` : '');
    ...
    useEffect(() => {
        ...
        if ((coverFileValue) || (avatarFileValue) || (nameValue && collection && nameValidity.validState && collection.name !== nameValue) || (bioValue && collection && bioValidity.validState && collection.bio !== bioValue) || (collection && explicitContentSelected !== collection.explicitContent)) {
    ...
    }, [coverFileValue, avatarFileValue, nameValue, nameValidity, bioValue, bioValidity, explicitContentSelected, id, collection])
    ...
    function handleSubmit() {
        ...
        if (submitBtnActive) {
            ...
            console.log('__________')
            pythonApi.updateCollection({ data: formData })
                .then((res) => {
                    setTimeout(() => {
                        let curUser = currentUser
                        curUser.collections = curUser.collections.map((item) => {
                            if (item._id === res._id) return res
                            else return item
                        })
                        console.log(curUser.collections.map((item) => {
                            if (item._id === res._id) return res
                            else return item
                        }))
    ...
    // useEffect(() => {
    //     if (id && currentUser) {
    //         let collect = currentUser.collections.filter((item) => {
    ...
    //             setCoverFileUrlValue(`${API_LINK}/collections/get_file/${collect.cover_url.size0}`)
    //             setCoverFileValidity({
    //                 errorMassage: '',
    //                 validState: true
    //             });
    //             setAvatarFileUrlValue(`${API_LINK}/collections/get_file/${collect.avatar_url.size4}`)
    //             setAvatarFileValidity({
    //                 errorMassage: '',
    //                 validState: true
    //             });
    //             setNameValue(collect.name)
    //             setNameValidity({
    //                 errorMassage: '',
    //                 validState: true
    //             });
    //             setbBioValue(collect.bio)
    //             setbBioValidity({
    //                 errorMassage: '',
    //                 validState: true
    //             });
    //             setExplicitContentSelected(collect.explicitContent)
    //         }
        ...
        if (id && currentUser) {
            ...
            pythonApi.getCollection({ collection_id: id })
                .then((res) => {
                    ...
                    if (currentUser._id === res.creator_id) {
                        setCollection(res)
                        setCoverFileUrlValue(`${API_LINK}/collections/get_file/${res.cover_url.size0}`)
                        setCoverFileValidity({
                            errorMassage: '',
                            validState: true
                        });
                        setAvatarFileUrlValue(`${API_LINK}/collections/get_file/${res.avatar_url.size4}`)
                        setAvatarFileValidity({
                            errorMassage: '',
                            validState: true
                        });
                        ...
                        setbBioValue(res.bio)
                        setbBioValidity({
                            errorMassage: '',
                            validState: true
                        });
                        setExplicitContentSelected(res.explicitContent)
                    } else {
                        navigate(`/`)
                    }
    ...
    return (
        <div className='create-collection'>
            <MetaTags>
                ...
                <meta property="og:title" content={`Edit collection`} />
                <meta property="twitter:title" content={`Edit collection`} />
                <meta property="vk:title" content={`Edit collection`} />
            </MetaTags>
            <div className='create-collection__container'>
                <h2 className='create-collection__title'>Edit collection</h2>
                <div className='create-collection__cover'>
                    ...
                    <div className={`create-collection__cover-box ${coverFileValidity.errorMassage ? 'create-collection__cover-box_error' : ''}`}>
                        {coverFileUrlValue ?
                            <img className='create-collection__cover-box-img' src={coverFileUrlValue} alt=''></img>
                            :
                ...
                <div className='create-collection__avatar'>
                    ...
                    <div className={`create-collection__avatar-box ${avatarFileValidity.errorMassage ? 'create-collection__avatar-box_error' : ''}`}>
                        {avatarFileUrlValue ?
                            <img className='create-collection__avatar-box-img' src={avatarFileUrlValue} alt=''></img>
                            :
                            <svg className='create-collection__cover-box-icon' width="58" height="33" viewBox="0 0 58 33" fill="none" xmlns="http://www.w3.org/2000/svg">
                ...
                <div className='create-collection__text-inputs'>
                    ...
                    <div className={`create-collection__submit-btn ${submitBtnActive ? 'create-collection__submit-btn_active' : ''}`} onClick={() => {
                        ...
                        {submitPreloader ? <MiniPreloader /> : <p className='create-collection__submit-btn-text'>{submitFine ? 'Successfully updated' : 'Update collection'}</p>}
